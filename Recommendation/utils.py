...
def load_sbert_model():
    cached_model = cache.get('sbert_model')
    if cached_model:
        return cached_model
    try:
...
def recommend_opportunities(user):
    user_vector = get_user_resume_vector(user, sbert_model)
    ...
    opportunities_data = get_opportunities_with_vectors(sbert_model)
...
def recommend_users_for_opportunity(opportunity):
    embeddings = sbert_model
    opp_vector = get_opportunity_vector(opportunity, embeddings)
    if np.linalg.norm(opp_vector) == 0:
        logger.warning(f"Opportunity {opportunity.opportunity_name} has no valid data.")
        return []

    all_users = get_all_users_with_vectors(embeddings)
    scores = []
